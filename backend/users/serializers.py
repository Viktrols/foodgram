...
class FollowSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if (self.context.get('request').method == 'DELETE'
                and not Follow.objects.filter( user=user, following__id=following_id).exists()):
            raise serializers.ValidationError('Вы не были подписаны на этого пользователя')
        return data
...
class ShowFollowSerializer(serializers.ModelSerializer):
    ...
    def get_recipes(self, obj):        
        ...
        return ShowRecipeLightSerializer(recipes, many=True, context=context).data
